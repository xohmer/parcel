...
describe.skip('plugins', function() {
  test('should load plugins and apply custom asset type', async () => {
    let b = await bundle(
  ...
  test('should load package.json from parent tree', async () => {
    let b = await bundle(
      path.join(
        __dirname,
  ...
  test('log a warning if a plugin throws an exception during initialization', async () => {
  ...
  test('log a warning if a parser throws an exception during initialization', async () => {
    sinon.stub(logger, 'warn');
